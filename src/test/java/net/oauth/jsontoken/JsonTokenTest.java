...
public class JsonTokenTest extends JsonTokenTestBase {
  ...
  private static final Duration SKEW = Duration.ofMinutes(1);
  ...
  public void setUp() throws Exception {
    super.setUp();
    clock.setNow(Instant.ofEpochSecond(1276669722000L));
  }
  ...
  public void testFutureToken() throws Exception {
    Instant issuedAt = clock.now().plus(SKEW.plus(Duration.ofSeconds(1)));
    Instant expiration = issuedAt.plus(Duration.ofDays(1));
    checkTimeFrameIllegalStateException(issuedAt, expiration);
  }

  public void testPastToken() throws Exception {
    Instant expiration = clock.now().minus(SKEW.plus(Duration.ofSeconds(1)));
    Instant issuedAt = expiration.minus(Duration.ofDays(1));
    checkTimeFrameIllegalStateException(issuedAt, expiration);
  }
