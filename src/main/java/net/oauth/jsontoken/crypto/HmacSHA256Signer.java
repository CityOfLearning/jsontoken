...
public class HmacSHA256Signer extends AbstractSigner {
	 ...
	 *
	 * @param issuer
	 *            the id of this signer, to be included in the envelope of the
	 *            JSON token.
	 * @param keyId
	 *            the id of the key that will be included in the envelope. If
	 *            null, will be omitted from the envelope.
	 * @param keyBytes
	 *            the actual key.
	 * @throws InvalidKeyException
	 *             if the key cannot be used as an HMAC key.
	 */
	public HmacSHA256Signer(String issuer, String keyId, byte[] keyBytes) throws InvalidKeyException {
		super(issuer, keyId);

		signingKey = new SecretKeySpec(keyBytes, HMAC_SHA256_ALG);
		try {
		...
		// just to make sure we catch invalid keys early, let's initialize the
		// hmac and throw if something goes wrong
